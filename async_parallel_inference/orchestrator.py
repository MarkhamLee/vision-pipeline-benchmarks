...
class AsyncOrchestrator(BaseOrchestrator):
    ...
    async def _frame_loader(self,
        ...
        try:
            for item in source.frames():
                ...
                frame = (item['frame']
                         if isinstance(item, dict) and item.get('event') == 'frame'  # noqa: E501
                         else item)
        ...
        while True:
            ...
            try:
                ...
                if 'model1' in bucket and 'model2' in bucket:
                    ...
                    interval['model1_latencies_ms'].append(m1.worker_latency_ms)  # noqa: E501
                    interval['model2_latencies_ms'].append(m2.worker_latency_ms)  # noqa: E501
                    interval['queue_wait_ms'].append(
                        max(m1.queue_wait_ms, m2.queue_wait_ms))
    ...
    async def _flush(self, interval: dict, elapsed_s: float) -> None:
        ...
        effective_inference_fps = (frame_count / total_inference_time_s
                                   if total_inference_time_s > 0 else 0.0)
        effective_pipeline_fps = (frame_count / elapsed_s
                                  if elapsed_s > 0 else 0.0)
        avg_combined_inference_latency_ms = round(
            avg_model1_latency_ms + avg_model2_latency_ms, 3)

        queue_1_depth = (self._frame_queue_1.qsize()
                         if self._frame_queue_1 else -1)
        queue_2_depth = (self._frame_queue_2.qsize()
                         if self._frame_queue_2 else -1)
    ...
    def _finalize_video(self) -> None:
        ...
        summary = {
            ...
            'avg_combined_inference_latency_ms': round(self._safe_avg(
                model1_latency_sum_ms + model2_latency_sum_ms, frame_count), 3),  # noqa: E501
        ...
        logger.info(
            'Video completed | %s | frames=%d | infer_fps=%.2f | pipeline_fps=%.2f',  # noqa: E501
            summary['video_name'],
