...
A high level overview of how the asynchronous benchmark works is available [here](../async_parallel_inference_mobile_4060/readme.md)
...
* The benchmarks weren't run under ideal conditions, i.e., numerous background apps were running. The reason for this is that an edge inference server is often doing numerous other tasks in addition to running models: recording video feeds, sending alerts if a camera has an issue, collecting sensor data, uploading files, in addition to running several models in parallel. I.e., running tests under ideal conditions wouldn't provide useful data for future edge deployments 
* One aspect of the benchmark that doesn't exactly map to the "real world" is that the InfluxDB and Postgres databases are running on the same local network as the inference server, meaning the latency is quite a bit less than it would be if the databases were hosted in the cloud. Future iterations of the benchmarks will compensate for this by adding an artificial delay and/or using cloud based database instances.
